...
async function showAvailableRecipes() {
    ...
    for (const recipeData of recipes) {
        ...
        recipeContainer.className = 'card overflow-auto p-2 m-2 col-sm-4'

        /* Heading */
        const headingElement = document.createElement('h4')
        headingElement.className = 'card-title text-center font-weight-bold'
        headingElement.textContent = mealData.RECIPE_NAME
        recipeContainer.appendChild(headingElement)

        /* Ingredients table */
        const tableElement = document.createElement('table')
        tableElement.className = 'table'
        recipeContainer.appendChild(tableElement)
        ...
        for(const config of additionalContentConfig){
            ...
            recipeContainer.appendChild(additionalElement)
        }
